...
def main():
    ...
    while True:
        ...
        if s == "quit":
            break
        print("The message send to switch is : ", s)
        try:
            i,ts = p(s,0,[])
            # Construct packet using parsed tokens
            pkt = Ether(dst='00:00:00:00:01:02', type=0x1234) / P4calc(op=ts[1].value,
            ...
            if resp:
                p4calc=resp[P4calc]
                if p4calc:
                    print("The result send back from server : ", p4calc.result)  # Print the result from the response packet
                else:
